...
for decoder in decoders:
    for count, phase in enumerate(intervals):
        for alevel in alevels:
            ...
            # Data
            pvals = hlp.pvals_from_csv(pval_path, only_significant=True, decode_for=decode_for, alevel=alevel)
            ...
            for area in unique_areas:
                ...
                if decode_for == 'resp':
                    title_string = ('Decoding responses in the ' + phase.replace('_500','') + ' period')
                else:
                    title_string = ('Decoding stimulus class in the ' + phase.replace('_500','') + ' period')
                plt.title(title_string, fontsize=60, family='Helvetica Neue LT')
                plt.savefig(path_out + sess + '_' + decode_for + '_0' + str(count+1) + phase + '_' + str(alevel) + '.png')
