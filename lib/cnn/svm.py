...
def train_classif(x_train, y_train, kernel='linear', c=1, gamma=1):
    # Convert one-hot back to simple class labels
    y_train = np.where(y_train == 1)[1]
    clf = SVC(C=c, gamma=gamma, kernel=kernel)
    clf.fit(x_train, y_train)
    return clf

def eval_classif(clf, x_test, y_test):
    # Convert one-hot back to simple class labels
